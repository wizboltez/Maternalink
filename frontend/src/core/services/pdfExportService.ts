...
export async function printAllSessionsPdf(): Promise<void> {
  try {
    const token = contractionApi.getAuthToken();
    if (!token) {
      Alert.alert('Export Failed', 'You must be signed in to export a report.');
    ...
    const fileName = 'maternalink_full_history.pdf';
    const cachePath = `${ReactNativeBlobUtil.fs.dirs.CacheDir}/${fileName}`;
    ...
    const response = await ReactNativeBlobUtil.config({
      ...
      fileCache: true,
      appendExt: 'pdf',
    }).fetch('GET', `${API_BASE_URL}/monitoring/export/pdf-all`, {
      Authorization: `Bearer ${token}`,
      Accept: 'application/pdf',
    });
    ...
    if (Platform.OS === 'android') {
      await RNPrint.print({ filePath, jobName: 'Maternalink Full Session History' });
    } else {
      await RNPrint.print({ filePath });
    }
  } catch (err: any) {
    Alert.alert(
      'PDF Export Failed',
      err?.message || 'Unable to generate or print the combined session report.'
    );
  }
}
